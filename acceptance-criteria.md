...
- [ ] Reporter cannot see assignedTo in ticket UI or rendered page output (FR-19, FR-45, EC-7, ISS-1)
...
- [ ] Closed and Cancelled tickets remain viewable on the detail page with read-only fields and no write forms (FR-22, A-15, ISS-8)
...
- [ ] User can edit their own comment message while the parent ticket is not Closed or Cancelled (FR-33, A-6, ISS-5)
- [ ] Comment deletion is not available for any role (C-2, A-13)
- [ ] Comments cannot be added on Closed or Cancelled tickets (FR-34)
- [ ] Existing comments cannot be edited on Closed or Cancelled tickets (FR-34, ISS-5, EC-5)
- [ ] Keyword search matches ticket title and description (FR-36, A-14, ISS-6)
...
- [ ] Edit or comment on Closed/Cancelled ticket returns rejection (FR-22, FR-34, EC-5, ISS-5, ISS-8)
...
- [ ] Concurrent update after ticket moved to Closed or Cancelled is rejected via status re-check on form submit (EC-13, NFR-6, ISS-4)
